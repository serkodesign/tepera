...
// PUB-6 (SRS): upload-keystore для Play App Signing. `key.properties` (і сам `.jks`) НІКОЛИ не
// комітяться (.gitignore) — файл існує лише локально й у резервній копії розробника. Білд без
// нього (напр. свіжий клон репозиторію) просто не підписує release-варіант, а не падає.
